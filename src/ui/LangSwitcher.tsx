...
export const LangSwitcher = ({ lng }: { lng: Lng }) => {
  const pathname = usePathname();

  return (
    <div className="flex w-36 items-center space-x-2 rounded-xl border border-zinc-800 bg-black px-4 py-3 text-zinc-600">
      <svg
      ...
      <Link
        href={`/${lng === 'ja' ? 'en' : 'ja'}/${pathname?.split('/').slice(2)}`}
        className="text-sm font-medium hover:text-zinc-100"
      >
        {lng === 'ja' ? 'English' : '日本語'}
      </Link>
    </div>
